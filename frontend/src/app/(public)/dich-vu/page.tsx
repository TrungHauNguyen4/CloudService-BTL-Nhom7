    ...
    apiClient.get('/public/service-plans')
      .then(res => {
        if (res.data && res.data.length > 0) {
          ...
          const mappedServices = res.data.map((plan: any, index: number) => {
            const fallbackStyle = fallbackServices[index % fallbackServices.length];
            return {
              id: plan.id, // Must use ID because backend expects GUID
              name: plan.name,
              desc: plan.category?.name || 'Gói dịch vụ cao cấp',
              price: new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(plan.monthlyPrice || 0).replace('₫', 'đ'),
              features: plan.specs ? plan.specs.split('\n').filter((s: string) => s.trim() !== '') : fallbackStyle.features,
              icon: fallbackStyle.icon,
              color: fallbackStyle.color,
              bgIcon: fallbackStyle.bgIcon,
            };
          });
      ...
      {!loading && (
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          {services.map((svc, index) => (
            <div key={index} className="group relative bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 z-10 overflow-hidden flex flex-col h-full">
              ...
              <div className="space-y-3 mb-8">
                {svc.features.map((feature: string, idx: number) => (
